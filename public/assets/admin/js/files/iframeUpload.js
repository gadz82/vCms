...
(function(yourcode){
    ...
    $('document').ready(function(){
        ...
        function postCompleteUpload(){
            var parent_url = (window.location != window.parent.location) ? document.referrer : document.location.href;
            $.postMessage({action : 'upload_complete', value : 'ciao'} , parent_url, parent );

        }

        function postResize(restore){
            var parent_url = (window.location != window.parent.location) ? document.referrer : document.location.href;
            if(restore == null){
                $.postMessage({action : 'enlarge_upload', value : $('body').outerHeight(true)} , parent_url, parent );
            } else {
                $.postMessage({action : 'restore_upload', value : $('body').outerHeight(true)} , parent_url, parent );
            }
        }
    });
}));