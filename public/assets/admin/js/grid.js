$(document).ready(function(){
	...
	grid.on('click','.btn_grid',function(){
		...
		}else if($(this).hasClass('grid_clone')){

			window.location.href = jqGrid_init.entityActions.clone_url+'/'+id;
		...
		}else if($(this).hasClass('grid_subgrid')){
			
			grid_subgrid.jqGrid('setGridParam',{
				url: jqGrid_init.entityActions.search_url,
				postData: { id: id },
				datatype:'json'
			});
			grid_subgrid.jqGrid('setCaption', 'Dettagio :: '+id);
			grid_subgrid.trigger("reloadGrid");
			
		}else if($(this).hasClass('grid_save')){

			$('.grid_save, .grid_cancel',div).hide();
			$('.grid_edit',div).show();
			grid.jqGrid('saveRow',id, {
				successfunc : function(res){
					if(res.responseJSON.success !== undefined){
						$('<div class="alert alert-success alert-dismissable">'+res.responseJSON.success+'</div>').appendTo('.content-header').slideDown('fast').delay(3000).slideUp('fast', function(){ $(this).remove(); });
						return true;
					}
				},
				errorfunc : function(err){
					$('<div class="alert alert-danger alert-dismissable">Si &egrave; verificato un errore durante l\'aggiornamento!</div>').appendTo('.content-header').slideDown('fast').delay(3000).slideUp('fast', function(){ $(this).remove(); });
					resetInlineEdit(id,div);
				}
			});
			
		}else if($(this).hasClass('grid_cancel')){
			resetInlineEdit(id, div);			
	...
	colModels.push({ label:'Azioni', name:'azioni', sortable:false,
		formatter: function(cellvalue, options, rowObject){
			...
			if(jqGrid_init.gridActions.indexOf('clone') > -1){
				r+= '<div class="btn_grid grid_clone" title="Clona"><span class="fa fa-copy fa-fw"></span></div>';
			}
			...
			if(jqGrid_init.gridActions.indexOf('detail') > -1){
				r+= '<div class="btn_grid grid_detail" title="Dettaglio"><span class="fa fa-search fa-fw"></span></div>'
			}
			if(jqGrid_init.gridActions.indexOf('edit_inline') > -1){
				r+= '<div class="btn_grid grid_edit" title="Modifica veloce"><span class="fa fa-magic fa-fw"></span></div>';
				r+= '<div class="btn_grid grid_save" title="Salva"><span class="fa fa-check fa-fw"></span></div>';
				r+= '<div class="btn_grid grid_cancel" title="Annulla"><span class="fa fa-remove fa-fw"></span></div>';
			}
			r+= '</div>';
    ...
    grid.jqGrid({
        ...
        colModel: colModels,
        url: jqGrid_init.entityActions.search_url,
        postData: { form_search: function(){ return $('#form_'+entity).serialize(); } },
        mtype: 'POST',
        datatype: 'json',
