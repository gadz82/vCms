...
(function($){
		...
		lazyLoad: function() {
			...
			if( lazyLoadEl.length > 0 ) {
				lazyLoadEl.each( function(){
					...
					element.attr( 'src', '/assets/site/images/blank.svg' ).css({ 'background': 'url(/assets/site/images/preloader.gif) no-repeat center center #FFF' });
	...
	SEMICOLON.documentOnLoad = {
		...
		init: function(){
			...
			$('form[id^="cms-form-"]').on('submit', function(e){
				e.preventDefault();
				var form = $(this),
				...
				$.ajax({
					...
					success: function (data, status, request){
						if(data.content !== undefined){
							form.find('.form_msg').append(data.content).fadeIn('fast');
						...
						updateCSRF(request.getResponseHeader('csrfRKey'),request.getResponseHeader('csrfRVal'));
					},
			...
			function updateCSRF(k,v){
				$('input#csrf').each(function(){
					$(this).attr('name', k);
					$(this).val(v);
				});
			}
			...
			$('#login-form').on('submit', function(e){
				...
				form.find('.form_msg').empty();
				submit.after(loader);
				var data = "url=http://area-riservata.gustourconad.it/utenti.class.php?op=login&user="+userName.val() +"&password="+password.val();
				...
				$.ajax({
					...
					data: data,
					success: function(rs){
						if(rs.success && rs.msg !== undefined){
							form.find('.form_msg').append(rs.content).fadeIn('fast');
							if(rs.msg.length > 0){
								setTimeout(function () {
									window.location.href="http://area-riservata.gustourconad.it/home.php?nounce=" + rs.msg;
								},1500)
							}
						} else {
							form.find('.form_msg').append(rs.content).fadeIn('fast');
