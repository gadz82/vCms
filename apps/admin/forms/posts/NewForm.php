...
class NewForm extends Form {
    ...
    protected $id_applicazione = 1;
	protected $fields = array();
	protected $exclude_required = array();
	protected $custom_validation = array();

	public function initialize($entity=null, $options=array()){
		$this->id_tipologia_post = $this->getDI()->getSession()->get('id_tipologia_post_current');
		$this->id_applicazione = $this->getDI()->getSession()->get('id_applicazione');
		$exclude_fields['Posts'] = array('id','data_creazione','data_aggiornamento','id_utente','attivo');
		$order_fields['Posts'] = array('id_applicazione', 'id_tipologia_stato', 'id_tipologia_post', 'slug', 'data_inizio_pubblicazione', 'data_fine_pubblicazione', 'titolo', 'excerpt', 'testo');

        //Ai fini del js validator
		$post_optional_fields =  ['excerpt', 'data_inizio_pubblicazione', 'data_fine_pubblicazione', 'id_tipologia_stato', 'applicazione', 'tags'];
        ...
        $this->fields['Posts']['id_tipologia_post']
			->setLabel('Tipologia post')
			->setAttribute('grid_class', 'col-lg-6 col-sm-12')
			->setAttribute('position', 'side')
            ->setAttribute('value', $this->id_tipologia_post)
			->setAttribute('useEmpty', false);
    ...
    private function generateMetaBoxes(){
		...
		foreach($meta_groups as $meta){
			...
			$metafields = \Meta::find([
                'conditions' => 'id_meta_group = ?1',
                'bind' => [ 1 => $meta->id ],
                'order' => 'priorita ASC'
            ]);
			$this->fields[$meta->descrizione] = array();
        ...
        foreach($filtri_groups as $fgroup){
			...
			$this->fields['filtri'][$fgroup->descrizione] = array();
    ...
    private function prepareValidation($id_tipologia_stato){
        ...
        foreach($this->fields as $key => $val){
            $arr_exclude_required['default'][$key] = array();
        }
        $arr_exclude_required['default']['Posts'] = array('data_inizio_pubblicazione', 'data_fine_pubblicazione', 'excerpt');
    ...
    private function compileValidation($id_tipologia_stato, $arr_exclude_required){
        ...
        for($i=0;$i<$count;$i++){
            ...
            if(!array_key_exists($arr_model[$i], $this->custom_validation)){
                $this->custom_validation[$arr_model[$i]] = array();
            }
        }
    ...
    public function beforeValidation(){
        ...
        foreach($this->fields as $key => $val){
            if($key == 'Posts'){
                $this->fields[$key] = $this->addValidateControl($this->fields[$key], array(), array());
            }
        }
    }
