...
class EditForm extends Form {
	...
	public function initialize($entity=null, $options=array()){
		...
		$exclude_fields = array('id','data_creazione','data_aggiornamento','id_utente','attivo', 'id_form', 'id_post');
		$order_fields = array();
		
		$this->fields = $this->getAutoRenderByModel(new \FormRequests(), 'FormRequests', $exclude_fields, $order_fields, false);

        $letto = new Check('letto', array('class' => 'ichek'));
        $letto->setLabel('Segna come letto')->setAttribute('value', 'on')->setAttribute('grid_class', 'col-sm-6');
        if($entity->letto) $letto->setAttribute('checked', 'checked');
        $this->fields['letto'] = $letto;
