...
class PHPExcel_CachedObjectStorage_APC extends PHPExcel_CachedObjectStorage_CacheBase implements PHPExcel_CachedObjectStorage_ICache {
	...
	protected function _storeData() {
		if ($this->_currentCellIsDirty && ! empty ( $this->_currentObjectID )) {
			...
			if (! apcu_store ( $this->_cachePrefix . $this->_currentObjectID . '.cache', serialize ( $this->_currentObject ), $this->_cacheTime )) {
				$this->__destruct ();
...
/**
	...
	public function __destruct() {
		...
		foreach ( $cacheList as $cellID ) {
			apcu_delete ( $this->_cachePrefix . $cellID . '.cache' );
		}
	}
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet ( $pCoord )) {
			...
			$success = apcu_fetch ( $this->_cachePrefix . $pCoord . '.cache' );
			if ($success === FALSE) {
	...
	public function getCacheData($pCoord) {
		...
		if (parent::isDataSet ( $pCoord )) {
			$obj = apcu_fetch ( $this->_cachePrefix . $pCoord . '.cache' );
			if ($obj === FALSE) {
				// Entry no longer exists in APC, so clear it from the cache array
				parent::deleteCacheData ( $pCoord );
				throw new PHPExcel_Exception ( 'Cell entry ' . $pCoord . ' no longer exists in APC cache' );
	...
	public function deleteCacheData($pCoord) {
		...
		apcu_delete ( $this->_cachePrefix . $pCoord . '.cache' );
	...
	public function copyCellCollection(PHPExcel_Worksheet $parent) {
		...
		foreach ( $cacheList as $cellID ) {
			if ($cellID != $this->_currentObjectID) {
				$obj = apcu_fetch ( $this->_cachePrefix . $cellID . '.cache' );
				if ($obj === FALSE) {
					// Entry no longer exists in APC, so clear it from the cache array
					parent::deleteCacheData ( $cellID );
					throw new PHPExcel_Exception ( 'Cell entry ' . $cellID . ' no longer exists in APC' );
				}
				if (! apcu_store ( $newCachePrefix . $cellID . '.cache', $obj, $this->_cacheTime )) {
					$this->__destruct ();
