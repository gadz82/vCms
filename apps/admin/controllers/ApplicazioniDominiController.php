...
class ApplicazioniDominiController extends ControllerBase
{
    ...
    public function initialize()
    {
        ...
        $this->jqGrid_columns = [
            ['label' => 'Stato applicazionidomini', 'name' => 'id_tipologia_stato'],
            ['label' => 'Tipo applicazionidomini', 'name' => 'id_tipologia_applicazioni_domini']
        ];
    ...
    public function createAction()
    {
        ...
        if ($this->request->isAjax() == true) {
            ...
            if (!$applicazione_dominio->save()) {
                ...
                $rs = $this->view->getRender('applicazioni', 'applicazioniDominiList', ['domini' => $domini], function ($view) {
            ...
            if ($this->request->isPost()) {
                ...
                if ($form->isValid($params)) {
                    ...
                    if ($controller_data->save()) {
                        ...
                        $this->response->setJsonContent(['success' => $this->alert_messagge['successEdit'], 'data' => $rs]);
                    } else {
                        foreach ($controller_data->getMessages() as $message) {
                            $messages[] = $message;
                        }
                        $this->response->setJsonContent(['error' => $this->alert_messagge['failEdit']]);
                    }
                ...
                $rs = $this->view->getRender('applicazioni', 'applicazioniDominiEditForm', ['form_applicazione_dominio_edit' => $form], function ($view) {
                    $view->setViewsDir("../apps/admin/views/partials/");
                    $view->setRenderLevel(\Phalcon\Mvc\View::LEVEL_LAYOUT);
                });
