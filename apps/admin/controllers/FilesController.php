...
class FilesController extends ControllerBase {
	...
	public function initialize(){
		...
		$this->jqGrid_columns = array(
            array('label'=>'Nome File', 'name'=>'filename'),
            array('label'=>'Tipo File', 'name'=>'filetype'),
            array('label'=>'Alt', 'name'=>'alt'),
            array('label'=>'Data', 'name'=>'data_creazione'),
            array('label'=>'Url', 'name'=>'fileurl')
		);
    ...
    public function getFileListAction(){
        ...
        if($this->request->isAjax() == true){
            ...
            if(!$files){
                $this->response->setJsonContent(array('error' => $this->alert_messagge['notFound']));
                return $this->response;
            }

            $rs = $this->view->getRender('files', 'fileListAjax', array('files_list' => $files), function ($view){
    ...
    public function getFileAction($id){
        if($this->request->isAjax()){
            ...
            if($file){
                $this->response->setJsonContent(array('success' => true, 'content' => $file->toArray()));
            } else {
                $this->response->setJsonContent(array('success' => false));
            }
        } else {
            $this->response->setJsonContent(array('success' => false));
        }
	...
	public function updateFileInfoAction($id){
        ...
        if(!isset($params['key']) || !isset($params['value'])){
            ...
            $this->response->setJsonContent(array('error' => true, 'data' => 'missing_params'));
            return $this->response;
        }
        ...
        if(!$file){
            $this->response->setStatusCode(500);
            $this->response->setJsonContent(array('error' => true, 'data' => 'file not found'));
        } else {
            ...
            if($file->save()){
                ...
                $this->response->setStatusCode(500);
                $this->response->setJsonContent(array('error' => true, 'data' => $file->getMessages()));
            }
        }
        return $this->response;
    }
