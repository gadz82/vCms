...
class ApplicazioniRoutesController extends ControllerBase {
	...
	public function initialize(){
		...
		$this->jqGrid_columns = array(
				array('label'=>'Stato route applicazione', 'name'=>'id_tipologia_stato'),
				array('label'=>'Tipo route applicazione', 'name'=>'id_tipologia_route')
		);
	...
	public function createAction(){
        ...
        if (!$ApplicazioniRoutes->save ()) {
            ...
            $rs = $this->view->getRender('applicazioni', 'applicazioniRoutesList', array('routes' => $routes), function ($view){
	...
	public function editAction($id){
        if($this->request->isAjax()) {
            ...
            if (!$controller_data){
                return $this->response->setJsonContent(array('error' => $this->alert_messagge['failEditForm']));
            }
            ...
            if ($this->request->isPost()) {
                ...
                $rs = $this->view->getRender('applicazioni', 'applicazioniRoutesEditForm', array('form_applicazione_route_edit' => $form), function ($view){
                    $view->setViewsDir("../apps/admin/views/partials/");
                    $view->setRenderLevel(\Phalcon\Mvc\View::LEVEL_LAYOUT);
                });
