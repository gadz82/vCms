...
class ControllerBase extends Controller {
	...
	protected $jqGrid_columns = array ();
	...
	protected $grid_inline_editable = array ();
	...
	protected function jqGrid_init($entityId, $entityCaption, $columns, $columns_select_editoptions = array(), $parent_model_columns = ['id', 'descrizione']) {
		if (! empty ( $columns_select_editoptions )) {
			foreach ( $columns_select_editoptions as $key => $model ) {
				...
				$this->view->{$model} = $model::find ( array (
						'condition' => 'attivo = 1',
						'columns' => $col_param,
						'cache' => array (
                            'key' => $model . '-find',
